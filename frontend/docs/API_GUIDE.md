...
#### `GET /api/locations/map_markers/`
Get lightweight location data optimized for map display.

**Authentication:** Not required
**Cache:** 30 minutes (same for all users)
**Performance:** 2 database queries, ~30 bytes per location
...
**Use Case:** Displaying all location pins on a map. 1000 locations = ~30KB payload.

**Frontend Service:** `locationsApi.getMapMarkers()`
...
| `/api/locations/map_markers/` | 30 min | Same for all users | 60x faster |
...
### Pattern 1: Map View with Markers

```javascript
// Load all location markers for map display
async function loadMapMarkers() {
  const response = await fetch('/api/locations/map_markers/');
  const markers = await response.json();

  // markers = [{ id, name, latitude, longitude }, ...]
  return markers;  // Render ALL markers at once
}

// Show preview when user clicks a marker
async function showLocationPreview(locationId) {
  const response = await fetch(`/api/locations/${locationId}/info_panel/`);
  const preview = await response.json();

  // preview = { id, name, formatted_address, average_rating, review_count }
  displayInfoPanel(preview);
}
```

**Performance:** 1000 locations = ~30KB payload, loads in <1 second
...
Always use `/api/locations/map_markers/` for map displays, not the full location list endpoint.
